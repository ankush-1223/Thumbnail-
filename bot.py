...
class HealthCheck:
    ...
    def verify():
        ...
        if "--health-check" in sys.argv or "healthcheck" in sys.argv:
            print("HEALTH_CHECK_OK")  # Keyob expects this response
            sys.exit(0)
    ...
    async def keep_alive():
        ...
        while True:
            print("🫀 [Health] Active at", time.strftime("%Y-%m-%d %H:%M:%S"))
            await asyncio.sleep(300)  # Ping every 5 minutes

# ==================== BOT CONFIGURATION ==================== 
class BotConfig:
    # Credentials (from Keyob environment)
    API_ID = int(os.environ.get("API_ID", 0))
    API_HASH = os.environ.get("API_HASH", "")
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "")
    ADMIN_ID = int(os.environ.get("ADMIN_ID", 0))
    
    # Dynamic settings (configured via bot)
    SUDO_USERS: List[int] = []
    TARGET_CHAT: str = ""
    SOURCE_CHAT: str = ""
    SKIP_MSG: int = 0
...
async def handle_callbacks(client: Client, query: CallbackQuery):
    if query.data == "start_cloning":
        ...
        # Setup menu logic here
        pass
        
    elif query.data == "sudo_menu":
        # Sudo management logic here
        pass

# ==================== BOT LIFECYCLE ====================
@bot.on_startup()
async def startup(client: Client):
    HealthCheck.verify()  # Initial health check
...
▫️ Admin: {BotConfig.ADMIN_ID}
▫️ Ready to clone!
""")

if __name__ == "__main__":
    HealthCheck.verify()  # Double protection
    bot.run()
